...
try:
    if API_PROVIDER == "gemini":
        ...
        API_KEY = st.secrets["api"]["groq_token"]
...
def save_chat_turn(chat_id: int, user_id: int, question: str, query: Optional[str], response: str, mode: str):
    if not st.session_state.auth_db:
        return
    ...
    try:
        cursor.execute("""
            INSERT INTO chat_history (chat_id, user_id, question, query_generated, response, mode)
            VALUES (%s, %s, %s, %s, %s, %s)
...
def call_llm(system_prompt: str, user_prompt: str, temperature: float = 0.1, max_tokens: int = 1000) -> str:
    """Call LLM API using Groq"""
    if not API_KEY :
        st.error("Groq API not configured. Install 'groq' SDK and set 'api.groq_token' in secrets.")
        return None
...
def generate_sql_query(user_question: str, schema: str, context: str = "") -> Dict:
    """Generate SQL query from natural language with multi-table support"""
    ...
    # Detect if we have both catalog and sales tables
    has_catalog = any('catalog' in t.lower() for t in table_names)
    has_sales = any('sales' in t.lower() for t in table_names)
    
    catalog_table = next((t for t in table_names if 'catalog' in t.lower()), 'footwear_catalog')
    sales_table = next((t for t in table_names if 'sales' in t.lower()), 'footwear_sales')
    
    # ALWAYS use both tables if available (no keyword detection)
    use_both_tables = has_catalog and has_sales
    
    # Enhanced system prompt with multi-table examples
    if use_both_tables:
        few_shot_examples = f"""
MULTI-TABLE QUERY EXAMPLES - ALWAYS USE BOTH TABLES:

EXAMPLE 1 (Sales-related):
Question: Which product is the top selling?
Generated SQL: 
SELECT c.product_id, c.name, c.brand, c.price, SUM(s.quantity_sold) as total_sold, SUM(s.total_revenue) as total_revenue
FROM {catalog_table} c
JOIN {sales_table} s ON c.product_id = s.product_id
GROUP BY c.product_id, c.name, c.brand, c.price
ORDER BY total_sold DESC
LIMIT 10

EXAMPLE 2 (Sales-related):
Question: Show me best selling Nike shoes
Generated SQL:
SELECT c.product_id, c.name, c.brand, c.category, c.price, SUM(s.quantity_sold) as total_sold
FROM {catalog_table} c
JOIN {sales_table} s ON c.product_id = s.product_id
WHERE LOWER(c.brand) LIKE '%nike%'
GROUP BY c.product_id, c.name, c.brand, c.category, c.price
ORDER BY total_sold DESC
LIMIT 10

EXAMPLE 3 (Attribute filtering - still use JOIN):
Question: Show me white sneakers
Generated SQL:
SELECT c.product_id, c.name, c.brand, c.color, c.sub_category, c.price, SUM(s.quantity_sold) as total_sold
FROM {catalog_table} c
JOIN {sales_table} s ON c.product_id = s.product_id
WHERE LOWER(c.color) LIKE '%white%' AND c.sub_category = 'sneakers'
GROUP BY c.product_id, c.name, c.brand, c.color, c.sub_category, c.price
ORDER BY total_sold DESC
LIMIT 10

EXAMPLE 4 (Cumulative filters with JOIN):
Previous context: Active filters: color='white', sub_category='sneakers'
Question: I want size 11
Generated SQL:
SELECT c.product_id, c.name, c.brand, c.color, c.size, c.price, SUM(s.quantity_sold) as total_sold
FROM {catalog_table} c
JOIN {sales_table} s ON c.product_id = s.product_id
WHERE LOWER(c.color) LIKE '%white%' AND c.sub_category = 'sneakers' AND c.size = 11
GROUP BY c.product_id, c.name, c.brand, c.color, c.size, c.price
ORDER BY total_sold DESC
LIMIT 10

EXAMPLE 5 (Price range with JOIN):
Question: Show me shoes under 5000 rupees
Generated SQL:
SELECT c.product_id, c.name, c.brand, c.price, SUM(s.quantity_sold) as total_sold
FROM {catalog_table} c
JOIN {sales_table} s ON c.product_id = s.product_id
WHERE c.price < 5000
GROUP BY c.product_id, c.name, c.brand, c.price
...
CRITICAL RULES:
- ALWAYS JOIN {catalog_table} and {sales_table} using product_id
- ALWAYS include sales data: SUM(s.quantity_sold) as total_sold
- ALWAYS GROUP BY all selected catalog columns
- ALWAYS ORDER BY total_sold DESC (show most popular first)
- Apply filters on catalog table attributes in WHERE clause
- MAINTAIN cumulative filters from context
- For specific sales queries, also select SUM(s.total_revenue)"""

        system_prompt = f"""You are a SQL query generator for MySQL database with multiple tables.

{schema}

AVAILABLE TABLES:
- {catalog_table}: Product catalog (product details, attributes)
- {sales_table}: Sales transactions (quantity_sold, total_revenue, product_id)

{few_shot_examples}

MANDATORY RULES:
1. ALWAYS JOIN BOTH TABLES:
   → Use: {catalog_table} c JOIN {sales_table} s ON c.product_id = s.product_id
   → This is REQUIRED for ALL queries

2. ALWAYS AGGREGATE SALES DATA:
   → Include: SUM(s.quantity_sold) as total_sold
   → Optionally include: SUM(s.total_revenue) as total_revenue

3. ALWAYS USE GROUP BY:
   → Group by all catalog columns you select (e.g., c.product_id, c.name, c.brand, c.price)

4. ALWAYS ORDER BY SALES:
   → Use: ORDER BY total_sold DESC
   → This shows the most popular/best selling items first

5. APPLY FILTERS CUMULATIVELY:
   → Extract filters from === ACTIVE FILTERS === section
   → Apply them in WHERE clause on catalog table (e.g., c.color, c.size, c.brand)
   → Only modify/remove if user explicitly changes them

6. INCLUDE LIMIT:
   → Use LIMIT 10 or LIMIT 20 for reasonable result sets

7. EXACT TABLE NAMES:
   → Use: {catalog_table} and {sales_table}

8. TEXT MATCHING:
   → Use LOWER() with LIKE for text matching (e.g., LOWER(c.brand) LIKE '%nike%')

9. RETURN ONLY SQL:
   → No markdown formatting, no explanations, no extra text"""

    else:
        # Single table or no sales data - use original logic
        primary_table = table_names[0] if table_names else catalog_table
        
        few_shot_examples = f"""
EXAMPLE 1:
Previous context: User asked for white sneakers.
Last query: SELECT * FROM {primary_table} WHERE LOWER(color) LIKE '%white%' AND sub_category = 'sneakers'
...
REQUIRED FORMAT:
SELECT * FROM {primary_table} WHERE ..."""
...
Generate a SQL query:"""
    ...
    llm_output = call_llm(system_prompt, user_prompt, temperature=0.1, max_tokens=500)
    ...
    try:
        ...
        # Only fix table names for single-table scenarios (when JOIN is not present)
        if 'JOIN' not in query.upper():
            # This is a single-table query, apply fixes
            primary_table = table_names[0] if table_names else catalog_table
                                    ...
                                    if not result["success"]:
                                        ...
                                        with st.expander("🔍 View Query & Results"):
                                            ...
                                            with col3:
                                                ...
                                                efficiency = ((1 - context_stats['total_tokens'] / max(1, context_stats['total_messages'] * 150)) * 100)
