...
if 'custom_sqlite_path' not in st.session_state:
    st.session_state.custom_sqlite_path = None
...
def get_custom_mysql_connection():
    """Connect to custom MySQL database from secrets or session"""
    if "custom_database" not in st.secrets:
        st.warning("⚠️ No 'custom_database' config found in secrets.toml. Add it like: [custom_database] host='yourhost' port=3306 database='yourdb' user='user' password='pass' ssl_disabled=true")
        return None

    try:
        # Use provided SSL config, with option for ssl_disabled
        ssl_config = {
            'ssl_disabled': st.secrets["custom_database"].get("ssl_disabled", True),  # Default to True as per user example
            'ssl_verify_cert': not st.secrets["custom_database"].get("ssl_disabled", True),
            'ssl_ca': st.secrets["custom_database"].get("ssl_ca", ""),
            'ssl_verify_identity': not st.secrets["custom_database"].get("ssl_disabled", True),
        }
        connection = mysql.connector.connect(
            host=st.secrets["custom_database"]["host"],
            port=int(st.secrets["custom_database"]["port"]),
            database=st.secrets["custom_database"]["database"],
            user=st.secrets["custom_database"]["user"],
            password=st.secrets["custom_database"]["password"],
            connect_timeout=30,
            **ssl_config
        )
        return connection
...
def create_persistent_sqlite_from_file(file_bytes: bytes, filename: str, user_id: int) -> Tuple[bool, Optional[str], str]:
    ...
    try:
        ...
        st.session_state.custom_sqlite_path = db_path
...
with col3:
    if st.button("🚪 Logout", type="secondary"):
        # Clean up user-specific DB files on logout
        if st.session_state.custom_sqlite_path:
            try:
                if os.path.exists(st.session_state.custom_sqlite_path):
                    os.remove(st.session_state.custom_sqlite_path)
            except:
                pass
        st.session_state.logged_in = False
        st.session_state.user_id = None
...
with st.sidebar:
    ...
    if st.session_state.db_mode == "system":
        ...
        st.info("💡 Upload CSV/Excel to create persistent SQLite DB (stored on disk per user)")
        uploaded_db_file = st.file_uploader(
            "Upload File for Persistent SQLite",
            type=['csv', 'xlsx', 'xls'],
            key="sqlite_uploader",
        ...
        if uploaded_db_file and st.button("📤 Create Persistent SQLite DB", use_container_width=True):
            with st.spinner("Processing and saving to disk..."):
                ...
                if success:
                    ...
                    if st.session_state.custom_sqlite_path:
                        # Reload schema
                        conn = sqlite3.connect(st.session_state.custom_sqlite_path)
        ...
        st.markdown('<div class="custom-db-form">', unsafe_allow_html=True)
        st.info("🔌 Connect to Custom MySQL Host via secrets.toml\n\nExample config:\n```toml\n[custom_database]\n host = 'yourhost.com'\n port = 3306\n database = 'yourdb'\n user = 'youruser'\n password = 'yourpass'\n ssl_disabled = true\n```")
        if st.button("🔄 Connect to Custom MySQL", use_container_width=True):
            with st.spinner("Connecting..."):
                conn = get_custom_mysql_connection()
                if conn:
        ...
        if st.session_state.custom_mysql_connection:
            ...
            st.warning("⚠️ Connect first to use uploads")
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    if st.session_state.db_mode == "system":
    ...
    elif st.session_state.db_mode == "custom_sqlite" and st.session_state.custom_sqlite_path:
        conn = sqlite3.connect(st.session_state.custom_sqlite_path)
        schema_to_show = get_database_schema(conn)
        conn.close()
        db_name = f"Persistent SQLite: {os.path.basename(st.session_state.custom_sqlite_path)}"
        ...
        try:
            db_name = st.secrets["custom_database"]["database"]
        except:
            db_name = "Custom MySQL"
        ...
        with st.chat_message("assistant"):
            with st.spinner("🤔 Analyzing with Llama 3.3..."):
                ...
                if st.session_state.db_mode == "system":
                    ...
                    active_conn = sqlite3.connect(st.session_state.custom_sqlite_path)
