...
with st.sidebar:
    ...
    if schema_to_show:
        ...
        if st.session_state.use_system_db:
            ...
            st.info(f"🗄️ **Custom Database (SQLite)**")
    ...
    if user_question:
        ...
        with st.chat_message("assistant"):
            with st.spinner("🤔 Analyzing with Llama 3.3..."):
                ...
                if st.session_state.use_system_db:
                    # Use business MySQL database
                    active_conn = get_business_db_connection()
                    active_schema = st.session_state.business_schema
                    is_sqlite = False
                else:
                    # Use custom SQLite database
                    active_conn = st.session_state.temp_db_connection
                    active_schema = st.session_state.temp_schema
                    is_sqlite = True
                
                if not active_conn:
                    response = "⚠️ No database connected. Please select a database or upload a file."
                    st.error(response)
                    save_chat_turn(
                    ...
                    try:
                        ...
                        # Close MySQL connection if needed (SQLite connections stay open)
                        if not is_sqlite and active_conn:
                            try:
...
with col2:
    db_status = "System DB (MySQL)" if st.session_state.use_system_db else "Custom DB (SQLite)"
