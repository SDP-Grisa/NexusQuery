...
def display_product_summary(product: Dict, idx: int):
    ...
    product_key = f"product_{idx}_{name[:20]}"
    
    # Display as clickable card
    st.markdown(f"""
    <div class="product-card" onclick="document.getElementById('{product_key}').click()">
    ...
    # Hidden button to trigger detail view
    if st.button("View Details", key=product_key, type="secondary", use_container_width=True):
                            ...
                            if not result["success"]:
                                ...
                                if df is not None and not df.empty:
                                    ...
                                    if is_product_data and len(df) <= 20:
                                        ...
                                        st.markdown("*Click on any product to view full details*")
                                        
                                        for idx, row in df.iterrows():
                                            display_product_summary(row.to_dict(), idx)
                                    else:
